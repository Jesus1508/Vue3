...
describe('<ProductView />', () => {
  ...
  test('should render page with a product', () => {
    ...
    const customTexArea = wrapper.findAllComponents({ name: 'CustomTexArea' });
    ...
    customTexArea.forEach((textArea) => {
  ...
  test('should not called submit a form if data is invalid', async () => {
    ...
    const titleImput = wrapper.findComponent({ name: 'CustomInput' });

    titleImput.vm.$emit('update:modelValue', '');
